...
当前每个 DSD Runtime Session 持有独立 HAL 引擎，PCM lease 按设备 UID 保存；同一设备的控制由应用级仲裁协调，ABI 控制入口串行化。宿主统一等待旧 PCM/DSD 箔暂停并释放资源，再启动新独占输出。不同 DAC 可并发，跟随系统输出不参与独占仲裁。关闭仍须 stop、销毁 IOProc、恢复格式并释放 Hog。
...
DSD64/128/256、raw DFF、历史曲目与播放位置恢复均已通过实测。跨箔独占交接已实现按设备 UID 的修复，待实机回归；记住上次选择的 DAC 仍未完成。已有资源的 metadata 与封面未发现问题，不列补充事项。DST、正式插件安装等仍属后续范围；宿主继续拥有外部列表。
...
播放 Session 属于窗口，物理设备独占属于应用范围。当前按设备 UID 统一仲裁 PCM/DSD 交接；迁移服务后仍须保留这一规则。不能让每个窗口各自创建不受协调的 HAL 引擎并独立取得 Hog Mode。
...
- 多窗口：同一设备 UID 上的 PCM 与 DSD 独占输出互斥；宿主先等待旧箔暂停并释放，再启动新箔。不同 DAC 独立，跟随系统输出不参与抢占；Hi-Fi 以 client ID 防止旧箔释放新 lease；
- 退出独占时恢复原 nominal/physical/virtual format 并释放 Hog Mode。

自动化已覆盖契约向后解码、ABI 编译、设备 rate 枚举和三仓构建。普通 PCM 单曲已实测可切换设备，支持设备按音源采样率工作；设备目录缓存和无变化格式跳过已缩短切换等待，旧分段回调失效后也已实测可从切换位置续播。2026-09-07 用户报告的多个箔争用独占设备问题已完成修复（宿主 `7d6576d`、Hi-Fi `8ba3210`），自动化及构建通过，待本轮实机交接验收；其他已测场景未发现问题，不据此推定所有设备/素材组合已覆盖。记住上次选择的 DAC 仍未完成，需保存稳定 UID 并在恢复时重新验证兼容性。
...
1. 回归本轮跨箔独占修复：PCM/DSD 共用同设备暂停/释放屏障，不同 DAC 与系统默认并发互不抢占；代码已接通，硬件验收待补。
