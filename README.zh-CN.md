...
这还不是正式发布。DSD 仅通过 DoP 输出，不实现 DSD → PCM 转换；DoP 不可用时明确提示。普通 PCM 设备路由不受影响。尚未完成的工作包括DST / SACD 多声道、专项 metadata、Session 恢复、DSD128/256 硬件回归、进程隔离，以及可供应用内安装的签名与公证 GitHub Release。设备断开/占用/睡眠恢复已实现，待真实 DAC 手测。
