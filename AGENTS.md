...
Current Phase 0 scope is Stereo DSD64 DSF → DoP → CoreAudio HAL on real hardware. Do not treat DSD → PCM fallback, raw DFF / DST / SACD ISO playback, complete seek UI, or Registry install as already done. Read `docs/hifi-phase0-dsf-playback-handoff.md` before changing HAL, DoP, or device lifecycle code.
