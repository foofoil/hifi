...
Current scope is raw DSF/DFF and uncompressed stereo SACD ISO → DoP → CoreAudio HAL. Stereo DSD64 is hardware-verified for DSF/DFF and for uncompressed stereo SACD ISO (list, seek, sound, and two-track auto-advance on SMSL); DSD128 and DSD256 are also hardware-verified. 5.0 DFF uses native/5.1/7.1 DoP when the device exposes that carrier format, otherwise MLFT/MRGT stereo; multichannel hardware verification is deferred until a suitable surround DoP DAC is available. DSD playback is DoP-only: DSD → PCM fallback and Automatic / Prefer DoP / Always PCM policies are explicitly out of scope. If DoP is unavailable, stop and report the reason; never silently convert to PCM. Ordinary PCM routing remains independent. Do not treat DST, SACD multichannel, or Registry install as already done. Device unplug/busy/hog/sleep recovery has passed real-DAC verification. History track/position restore has passed user verification. Multi-window exclusive-device contention is a known unresolved issue; remembering the last selected DAC remains unfinished. Existing audio metadata has no reported issues; do not schedule dedicated metadata work without a concrete defect. Read `docs/hifi-phase0-dsf-playback-handoff.md` section 4 before changing HAL, DoP, SACD sector packing, or device lifecycle code.
