...
Not yet a shipping release. DSD playback is DoP-only; DSD → PCM conversion is out of scope. DSD64/128/256 and history track/position restore are hardware/user-verified. Multi-window exclusive-device contention is a known unresolved issue; remembering the last selected DAC remains unfinished. Other tested device scenarios have no reported issues. Existing metadata needs no additional work. Later scope includes DST / SACD multichannel, process-isolation evaluation, and signed/notarized Releases for in-app install.
